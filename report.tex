...
%	\end{wrapfigure}	
	...
	\section{Method1: Evaluating the Raw Voxelmorph Architecture}\label{subsec:method1}
	...
	\subsection{Results}
	We used this model to register a moving sample and found that it did not introduce any artifacts, as demonstrated in Figure~\ref{fig:method3_artifact}. This conclusion was confirmed by comparing the results to the reference image in Figure~\ref{fig:method2_artifact}. In addition to visually inspecting the results, we also evaluated the performance quantitatively using the metrics described in sections \ref{sec:quality} and \ref{sec:loss_metric}. Our method was compared to the larvalign as a baseline.
	
	\paragraph{Test Setup}
	In order to evaluate the performance of the larvalign and method 3 methods, the larvaling dataset is used as the test dataset and metrics such as VI, TI, and Global Mattes Mutual Scores are calculated for all images in the dataset. The results are presented in tables. Additionally, a summary of the average scores achieved by both methods is presented in the form of a table.
	
	Method 3 performed better than the previous two methods in terms of registering the moving samples with large and smaller deformations, as shown in Figure~\ref{fig:method3_pas} and Figure~\ref{fig:method3_fail}. However, there is room for improvement at the lower tip of the Ventral Nerve Cord (VNC) where the nerve entry points can be aligned more accurately.
