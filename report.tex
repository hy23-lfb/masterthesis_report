...
%	\end{wrapfigure}	
	...
	\begin{table}[h]
		...
		\caption{Mattes Mutual Information, VI Error Indicator, TI Error Indicator scores measured on "good" quality images from the \texttt{Larvalign dataset} registered using \texttt{Cascaded Vanilla Voxelmorph}.}
	...
	\begin{table}[h]
		...
		\caption{Mattes Mutual Information, VI Error Indicator, TI Error Indicator scores measured on "good" quality images from the \texttt{Larvalign dataset} registered using \texttt{Cascaded Landmark Guided Voxelmorph}.}
	...
	\begin{table}[h]
		...
		\caption{Mattes Mutual Information, VI Error Indicator, TI Error Indicator scores measured on "medium" quality images from the \texttt{Larvalign dataset} registered using \texttt{Cascaded Vanilla Voxelmorph}.}
	...
	\begin{table}[h]
		...
		\caption{Mattes Mutual Information, VI Error Indicator, TI Error Indicator scores measured on "medium" quality images from the \texttt{Larvalign dataset} registered using \texttt{Cascaded Landmark Guided Voxelmorph}.}
	...
	\begin{table}[h]
		...
		\caption{Mattes Mutual Information, VI Error Indicator, TI Error Indicator scores measured on "random" quality images from the \texttt{Larvalign dataset} registered using \texttt{Cascaded Vanilla Voxelmorph}.}
	...
	\begin{table}[h]
		...
		\caption{Mattes Mutual Information, VI Error Indicator, TI Error Indicator scores measured on "random" quality images from the \texttt{Larvalign dataset} registered using \texttt{Cascaded Landmark Guided Voxelmorph}.}
...
\end{comment}
	...
	\begin{table}[h!]
		\centering
		\input{resources/chapter5_fresh/Quality/Quality_Good.tex}
	...
	\begin{table}[h!]
		\centering
		\input{resources/chapter5_fresh/Quality/Quality_Medium.tex}
	...
	\begin{table}[h!]
		\centering
		\input{resources/chapter5_fresh/Quality/Quality_Random.tex}
	...
	The below landmark registration error (LRE) equation~\eqref{eqn:lre} explained in subsection~\nameref{subsec:quantitative} from chapter~\ref{chap:imp_and_data} showed how the landmark registration error (LRE) was computed.
	...
	\begin{table}[h]
		\centering
		\input{resources/chapter5_fresh/stats_per_landmark.tex}
	...
	\begin{table}[h!]
		\centering
		\input{resources/chapter5_fresh/stats_per_scan_output/mean_stats_per_scan.tex}
	...
	\begin{table}[H]
		\centering
		\input{resources/chapter7/VNC_LRE.tex}
	...
	\begin{table}[H]
		\centering
		\input{resources/chapter7/LRE_Visual/Brain45_LRE.tex}
	...
	\begin{figure}[h!]
		\centering
		\includegraphics[width=0.7\columnwidth]{resources/chapter7/LRE_Visual/24b09.png}
		\caption{Overlay of the maximum intensity projection of the fixed image with the maximum intensity projection of the registered \textit{larvalign} output and the maximum intensity projection of the registered \texttt{Cascaded Landmark Guided Voxelmorph} output for \texttt{larvalign failure 1}.}
		\label{fig:larvalign failure 1}
	\end{figure}
	
	\begin{figure}[h!]
		\centering
		\includegraphics[width=0.7\columnwidth]{resources/chapter7/LRE_Visual/53a06.png}
		\caption{Overlay of the maximum intensity projection of the fixed image with the maximum intensity projection of the registered \textit{larvalign} output and the maximum intensity projection of the registered \texttt{Cascaded Landmark Guided Voxelmorph} output for \texttt{larvalign failure 2}.}
	...
	Since the output registered image from both the models were of the size 128x128 in x, y directions, it was hard to visually say if there was any blurriness induced in the model's output trained on anisotropic scans. To address this, we did the following analysis. We made the assumption that the output of the model\_anisotropic is a blurrier version of the output of the model\_isotropic. To test this, we took the difference between the two images, expecting to see high frequency content, such as sharp edges, in the result Figure~\ref{fig:diff}. However, it is possible that the difference in registration between the two models is causing this result. To further investigate, we also took the difference between the original image and a Gaussian blurred version of the same image, and compared the two "difference" images visually using a false color scale in MATLAB. Both these images appeared similar, providing further evidence that the output from model\_anisotropic is actual introducing blur.
	
	\begin{figure}[h!]
		\centering
		\includegraphics[width=0.7\columnwidth]{resources/discussion7/MAX_np_C3_brain0_isotropic.png}
		\caption{Registered output from the isotropic model.}
		\label{fig:isotropic_output}
	\end{figure}

	\begin{figure}[h!]
		\centering
		\includegraphics[width=0.7\columnwidth]{resources/discussion7/MAX_np_C3_brain0_anisotropic.png}
		\caption{Registered output from the anisotropic model.}
		\label{fig:anisotropic_output}
	\end{figure}

	\begin{figure}[h!]
		\centering
		\includegraphics[width=0.7\columnwidth]{resources/discussion7/Diff.png}
		\caption{The difference in output between isotropic model and anisotropic model.}
		\label{fig:diff
	\end{figure}
