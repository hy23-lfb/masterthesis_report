...
%	\end{wrapfigure}	
	...
	\chapter{Implementations and Data Preperation}
    ...
    The whole process is described in the Figure~\ref{fig:block_method4} and the following pseudo code demonstrates how landmarks are propagated from stage 1 to stage 's' by utilizing the deformation model at each stage.
	...
	The two images focused on the inferior tip of the ventral nerve cord (VNC) show the moving image before and after registration. As can be seen, it appears that the structures in this area were erased after registration.
	...
	\begin{figure}[h!]
		...
		\caption{An example moving sample image with large deformations at the lower tip of Ventral Nerve Cord (VNC) where the Method2 produced an artifact on the registered output. Four images are plotted in a row using Matlab. The first image is the fixed image against which the registration is being performed. The second image is the moving image that is being registered. The third image is the result of the registration process, and the fourth image is an overlay of the registered image and the fixed image.}
	...
	Comparing Figure~\ref{fig:method4_combine} and Figure~\ref{fig:method3_combine}, it is clear that the addition of landmarks at the lower tip of the ventral nerve cord resulted in better registration accuracy for method 4 than for method 3.
	...
	We will record 3 Mattes Mutual Information scores in total between the registered and the fixed image.
	\begin{enumerate}
	...
	The~\ref{table:lre} table below contains the summary information on the average of the mean registration errors for all 30 landmarks in the~\ref{table:lre_larvalign}, \ref{table:lre_vanilla}, and \ref{table:lre_ldm} tables.
	...
	The~\ref{table:quality_good}, ~\ref{table:quality_medium}, ~\ref{table:quality_random} tables below contains the summary information on the average of MMI, VI, and TI scores for the "good", "medium", and "random" quality images.
